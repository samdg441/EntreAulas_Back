...
export class AiService {
  ...
  static async summarizeOpenResponses(responses: string[], context: SummaryContext = 'profesor'): Promise<AiSummaryResult> {
    ...
    const providers: AiSummaryProvider[] = [
      ...
      new LocalSummaryProvider((texts) => ({
        summary: generateSmartSummary(texts),
        topics: extractTopKeywords(texts, 10),
    ...
    const combinedTopics = [
      ...new Set([...(result?.topics || []), ...topics.slice(0, 5)]),
    ].slice(0, 10)

    return {
      summary: result?.summary || generateSmartSummary(responses),
      topics: combinedTopics.length > 0 ? combinedTopics : topics,
      acosoDetectado,

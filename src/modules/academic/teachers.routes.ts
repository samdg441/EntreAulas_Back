...
import {
  listTeachersWithCourses,
  TeachersListHttpError,
} from './teachers-list.service'
...
router.get('/', authenticateToken, async (req: any, res) => {
  try {
    const teachers = await listTeachersWithCourses(req.user)
    return res.json(teachers)
  } catch (error) {
    if (error instanceof TeachersListHttpError) {

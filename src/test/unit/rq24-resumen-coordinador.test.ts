...
class RQ24ResumenCoordinador {
  ...
  FALLA_C1_estudianteEsCoordinador() {
    expect(esCoordinador(estudianteUser)).toBe(true)
  }

  FALLA_C8_searchNoFiltra() {
    const r = armarResumenCoordinador({
      profesores: [
        { id: 7, usuario_id: 'u1' },
        { id: 8, usuario_id: 'u2' },
    ...
    expect(r.teachers).toHaveLength(2)
  }
}
...
describe('RQ24 — Ver resumen del coordinador', () => {
  ...
  it('C10: notas 0 o negativas no entran al promedio', () => pruebas.C10_calificacionesInvalidas())
  it('FALLA C1: estudiante — se espera (mal) que sea coordinador', () =>
    pruebas.FALLA_C1_estudianteEsCoordinador())
  it('FALLA C8: search ana — se espera (mal) 2 docentes', () => pruebas.FALLA_C8_searchNoFiltra())
})
